...
public class MainScript
{
    ...
    */
        ...
        for (int ii = 0; ii < points.Count; ii++)
        {
            ...
            if (ii == 1 && impose_configuration == true)
            {
                ...
                TxRoboticViaLocationOperation Point = TxApplication.ActiveDocument.
                    GetObjectsByName(points[ii].Name.ToString())[0] as TxRoboticViaLocationOperation;
                var point_loc = Point as ITxRoboticLocationOperation;
                TxPoseData robot_config = ScanConfigurations(Point, robot);
                robot.CurrentPose = robot_config;
                point_loc.TeachCurrentRobotConfiguration();
                robot.CurrentPose = start_pose.PoseData;
            }
        }
    ...
    public static TxPoseData ScanConfigurations(TxRoboticViaLocationOperation point, TxRobot robot)
    {
        TxRobotInverseData inv = new TxRobotInverseData(point.AbsoluteLocation);
        ...
        {
            var pose = poses[i] as TxPoseData;
            robot.CurrentPose = pose;
            TxApplication.RefreshDisplay();
            double[,] J = CalculateJ();
            
            double detJ_new = CalculateDeterminant(J);
            double w = Math.Sqrt(Math.Abs(detJ_new));
            m_output.WriteLine("Determinant: " + w.ToString());
            if (w > detJ)
            {
                detJ = detJ_new;
                bestPose = pose;
                m_output.WriteLine("Best Pose: " + bestPose.ToString());
            }
    ...
    private static double[,] CalculateJ()
    {
